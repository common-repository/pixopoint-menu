...
function archivesmonthsdropdown() {
	...
	echo '">' . get_option('suckerfish_archivestitle') . '</a><ul>' , wp_get_archives('type=monthly') , '</ul></li>';
}
function archivesyearsdropdown() {
	...
	echo '<li'.$suckerfish_class.'><a href="';
	if (get_option('suckerfish_archivesurl') != '') {echo get_option('suckerfish_archivesurl');}
	echo '">' . get_option('suckerfish_archivestitle') . '</a><ul>' , wp_get_archives('type=yearly') , '</ul></li>';
}
		...
		foreach ($temp as $set) { // Splits array into two
			...
			if (($XXX == '' AND $pixo_which == 1) || ($XXX == 1 AND $pixo_which == 2)) { // Crude hack coz couldn't figure out how to finish a foreach early
				...
				if ($set[0] == 'search') {
					...
					<input type="text" value="" class="pixo_inputsearch" name="s" /> <input type="submit" value="'; _e('Search','pixopoint_theme');echo '" /> &nbsp;
					</form></li>';}
				// 'Home' option
				if ($set[0] == 'home') {home();}
				// 'Archive' option
