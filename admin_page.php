...
function show_suckerfish_options() {
	// Redirect to main site when editor loaded (note, seem to need to echo something immediately afterwards or it won't redirect - anyone know why?)
	if ($_POST['enableeditingpane'] == 'Load Editor') {
...
//	if (get_option('suckerfish_pageorder', 'Ascending Name')) {update_option('suckerfish_pageorder', 'Normal');} // Temporary fix for bug introduced to beta 0.5.8. Should be removed in later versions as shouldn't be necessary.
	...
	add_option('suckerfish_identification', rand(0, 999999));
...
<div class="wrap">
	<form method="post" action="options.php" id="options">
		...
		<div class="tabber" id="mytabber1">
			...
			<?php /* Home */ ?>
			<div class="tabbertab">
				<h2><?php _e('Home','pixopoint_theme'); ?></h2>
				<div class="clear"></div>
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
						<tr>
							<th scope="col"><?php _e('Menu contents','pixopoint_theme'); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col"><?php _e('Drag and drop the menu items to control what appears in your menu.','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
...
<div id="listItem_pages" class="pixo_widget">
	...
	<span class="handle"><?php _e('Pages','pixopoint_theme'); ?></span>
	<div id="pulldown_pages2" class="dropdown">
		<p>
			...
			<br />
			<label style="width:95px;margin-right:0;padding-right:0;"><?php _e('Single dropdown?','pixopoint_theme'); ?></label>
			<?php
			...
			<br />
			<label><?php _e('IDs of URLs to exclude','pixopoint_theme'); ?></label><br />
...
<div id="listItem_categories" class="pixo_widget">
	...
	<span class="handle"><?php _e('Categories','pixopoint_theme'); ?></span>
...
<div id="listItem_recentcomments" class="pixo_widget">
	...
	<span class="handle"><?php _e('Recent Comments','pixopoint_theme'); ?></span>
...
<div id="listItem_recentposts" class="pixo_widget">
	...
	<span class="handle"><?php _e('Recent Posts','pixopoint_theme'); ?></span>
...
<div id="listItem_home" class="pixo_widget">
	...
	<span class="handle"><?php _e('Home','pixopoint_theme'); ?></span>
...
<div id="listItem_archives" class="pixo_widget">
	...
	<span class="handle"><?php _e('Archives','pixopoint_theme'); ?></span>
		...
		<p>
			...
			<select style="float:left;" id="dropdownselect" name="suckerfish_archivesperiod">
				...
				switch ($suckerfish_archivesperiod){
					case "Years":echo '<option>Years</option><option>Months</option>';break;
					case "Months":echo '<option>Months</option><option>Years</option>';break;
					case "":echo '<option>Months</option><option>Years</option>';break;
				}
				?>
			</select>
		</p>
...
<div id="listItem_links" class="pixo_widget">
	...
	<span class="handle"><?php _e('Links','pixopoint_theme'); ?></span>
...
?>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
						<tr>
							<th scope="col"><?php _e('Menu design','pixopoint_theme'); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col"><?php _e('You can either disable the editing pane by using the close (X) button on the editor itself, or by disabling it here in the admin page.','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
							<td class="desc">
								<p><?php _e('You can edit the styling/design of your menu by loading the editing panel. Once you have turned on the editing panel on, then visit the main page of your site to edit your menu.','pixopoint_theme'); ?><br /><br /></p>
								<?php
									if (get_option('suckerfish_generator') == '') {update_option('suckerfish_generator', 'Plugin CSS');}
									if (get_option('suckerfish_generator') == 'Theme CSS') {echo '<p><strong>You have chosen to use your themes CSS to provide the menu styling. To disable this, please click the "Settings" tab and choose "Plugin CSS".</strong>';}
									elseif (get_option('suckerfish_editingpane') != 'on') {echo '<input class="loadeditorbutton" name="enableeditingpane" value="Load Editor" type="submit" />';}
									else {echo '<input class="loadeditorbutton" name="disableeditingpane" value="Disable editor" type="submit" />';}
								?>
								<br /><br /><br />
							</td>
						</tr>
			...
			<?php /* Settings tab */ ?>
			<div class="tabbertab">
				<h2><?php _e('Settings','pixopoint_theme'); ?></h2>
				<div class="clear"></div>
				<h3 class="yup">General Settings</h3>
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
						<tr>
							<th scope="col" colspan="2"><?php _e('Setting','pixopoint_theme'); ?></th>
							<th scope="col"><?php _e('Description','pixopoint_theme'); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col" colspan="3"><?php _e('You may modify your menus behaviour via the following settings','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<?php
							...
							<tr class="active">
					      	...
					      	<select name="suckerfish_generator">
								...
								<td class='name'><?php _e('Use theme or plugins CSS?','pixopoint_theme'); ?></td>
								<td class='desc'>
									<p><?php _e('You are currently using a theme designed to integrate directly with the PixoPoint Menu Plugin. You can either keep the existing menu design by using the themes CSS, or change to the plugins CSS to override the theme styling.','pixopoint_theme'); ?></p>
								</td>
							</tr>
						<?php } ?>
						<tr class="inactive">
							...
							<td class='name'><?php _e('Add a second menu?','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('You may add a second menu to your site. This is particularly common with magazine style layouts. For a second menu, add the following code to your theme <br /><code>&lt;?php if (function_exists(\'pixopoint_menu\')) {pixopoint_menu(2);} ?&gt;</code>.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class="inactive">
							<th scope='row' class='check-column'>
							...
							<td class='name'><?php _e('Remove title attribute?','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This removes the title attributes from the links in the menu. The title attributes display in most browsers as a small tool tip on hover over the links.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class="inactive">
							<th scope='row' class='check-column'>
						...
						<tr class='inactive'>
							...
							<td class='name'><?php _e('Style sheet','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('The plugin includes it\'s own built in stylesheet. However many site owners wish to use their themes built in stylesheet (good idea if you want to reduce your page size).','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<th scope='row' class='check-column'>
							...
							<td class='name'><?php _e('Auto theme support','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This option allows some themes, including <a href="http://themehybrid.com/">Theme Hybrid</a>, <a href="http://themeshaper.com/">Thematic</a> and Thesis to work automatically (no theme modifications necessary). It also works with many other themes we can\' guarantee it will work (it uses the wp_page_menu() function for most themes).','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<th scope='row' class='check-column'>
							...
							<td class='name'><?php _e('Compatibility mode','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This option disables a series of settings which can potentially cause conflicts with poorly coded themes or plugins.','pixopoint_theme'); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					...
					<tfoot>
						<tr>
							<th scope="col" colspan="3"><?php _e('The following settings allow you to modify the behaviour of your dropdown menus (if you have any)','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class='inactive'>
							...
							<td class='name'><?php _e('Speed of fade-in effect','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This option enhances the behaviour of the dropdown by creating an animated fade-in effect. This option is controlled by the <a href="http://users.tpg.com.au/j_birch/plugins/superfish/">\'Superfish plugin\'</a> for <a href="http://jquery.com/">jQuery</a>.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<th scope='row' class='check-column'>
							...
							<td class='name'><?php _e('Mouseover delay (milliseconds)','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This option adds a delay time before the dropdown appears. This option is controlled by the <a href="http://users.tpg.com.au/j_birch/plugins/superfish/">\'Superfish plugin\'</a> for jQuery.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<th scope='row' class='check-column'>
						...
						<tr class='inactive'>
							...
							<td class='name'><?php _e('Sensitivity','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('When this option is enabled, the menu will attempt to determine the user\'s intent. On low sensitivity, instead of immediately displaying the dropdown/flyout menu on mouseover, the menu will wait until the user\'s mouse slows down before displaying it.','pixopoint_theme'); ?></p>
							</td>
						</tr>
<?php /*	Needed to be removed as contained bug
						<tr class='inactive'>
							...
							<td class='name'><?php _e('Enable keyboard accessible menu?','pixopoint_theme'); ?></td>
							<td class='desc'>
								<p><?php _e('This option enables users to access your menu via the tab key on their keyboard rather than the mouse. Thanks to <a href="http://www.transientmonkey.com/">malcalevak</a> for writing the script. This option utilizes <a href="http://jquery.com/">jQuery</a>.','pixopoint_theme'); ?></p>
							</td>
						</tr>
*/ ?>
						<tr class='inactive'>
							...
							<td class='name'><?php _e('Enable arrow mark-up?','pixopoint_theme'); ?></td>
							<td class='desc'>
			...
			<?php /* Advanced tab */ ?>
			<div class="tabbertab">
				<h2><?php _e('Advanced','pixopoint_theme'); ?></h2>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('Order your WordPress pages here','pixopoint_theme'); ?>
					</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col"><?php _e('Special thanks to <a href="http://www.geekyweekly.com">froman118</a> for creating the <a href="http://www.geekyweekly.com/mypageorder">My Page Order plugin</a> which this part of the PixoPoint Menu plugin is based on.','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
							<td class="desc">
								<p><?php _e('You may modify the order of your WordPress pages here. Note: This only controls \'Pages\' and not the other various options in the plugin such as \'Categories\' and \'Recent Comments\'.','pixopoint_theme'); ?></p>
								<?php
									require('mypageorder.php');
									mypageorder();
								?>
								<p><br /><?php _e('To order your WordPress Categories, try installing the <a href="http://wordpress.org/extend/plugins/my-category-order/">My Category Order Plugin</a> which is able to do this.','pixopoint_theme'); ?><br /></p>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('My Page Order plugin','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
						<tr>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('You can manually modify your CSS for the main menu here.','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
						<th scope="col"><?php _e('Note that when you save the CSS in the editing pane it will overwrite your changes here','pixopoint_theme'); ?></th>
					</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('You can manually modify your CSS for the second menu here. Note that when you save the CSS in the editing pane it will overwrite your changes here.','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('Use this option to add extra CSS to the plugin.This CSS will <strong>not</strong> be modified when using the editing panel.','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
						<th scope="col"><?php _e('This CSS will <strong>not</strong> be modified when using the editing panel','pixopoint_theme'); ?></th>
					</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
				...
				if (function_exists('pixopoint_theme_advanced_customfields')) { ?>
				<div class="clear"></div>
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('This section is under development and is part of the advanced addon module.','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
						<th scope="col"><?php _e('text goes here','pixopoint_theme'); ?></th>
					</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
							<td class="desc">
								<p><?php pixopoint_theme_advanced_customfields(); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<?php /* Help tab */ ?>
			<div class="tabbertab">
				<h2><?php _e('Help','pixopoint_theme'); ?></h2>
				<div class="clear"></div>
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('Installation','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
						<th scope="col"><?php _e('Thanks for using our plugin :)','pixopoint_theme'); ?></th>
					</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
							<td class="desc">
								<p><?php _e('Add the following code wherever you want the menu to appear in your theme (usually header.php)','pixopoint_theme'); ?></p>
								<p><code><?php _e('&;t;?php if (function_exists(\'pixopoint_menu\')) {pixopoint_menu();}; ?&gt;','pixopoint_theme'); ?></code></p>

								<h4><?php _e('Getting it working with your theme','pixopoint_theme'); ?></h4>
								<p>
									<?php _e('Some of the more popular themes such as <a href="http://themehybrid.com/">Theme Hybrid</a>, <a href="http://themeshaper.com/">Thematic</a> and Thesis will work automatically when the \'Auto theme support\' option is chosen under the settings tab.','pixopoint_theme'); ?>
								</p>
								<p><?php _e('The following is an example of the code modified in the WordPress Default (Kubrick) theme:','pixopoint_theme'); ?><br />
								<code>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
					<tr>
						<th scope="col"><?php _e('Support','pixopoint_theme'); ?></th>
					</tr>
					</thead>
					<tfoot>
					<tr>
						<th scope="col"><?php _e('Please do not email us directly or via our <a href="http://pixopoint.com/contact/">contact form</a> for unpaid support. We prefer to offer support in our <a href="http://pixopoint.com/forum/">forum</a> so that others may learn from the advice','pixopoint_theme'); ?></th>
					</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class="inactive">
							<td class="desc">
								...
								<h4><?php _e('The easy way ... ','pixopoint_theme'); ?></h4>
								<p><?php _e('The easiest way to setup the PixoPoint Menu Plugin is to use it with a theme specifically designed to support the plugin. All themes exported from the <a href="http://pixopoint.com/generator/">PixoPoint Template Generator</a> (which have dropdown menus) support this by default. Simply activate your theme, then activate the plugin and the new menu will appear instantly. Visit the <a href="http://pixopoint.com/generator/">PixoPoint Template Generator</a> to get your own pre-supported theme.','pixopoint_theme'); ?></p>
								<p><?php _e('If you don\'t take this route then read on for simple instructions on how to integrate it into other themes ...','pixopoint_theme'); ?></p>
								<?php } ?>
								<h4><?php _e('Paid support','pixopoint_theme'); ?></h4>
								<p><?php _e('For direct help via the plugin author, please sign up for our Premium Support service ... <strong><a href="http://pixopoint.com/premium-support/">http://pixopoint.com/premium-support/</a></strong>','pixopoint_theme'); ?></p>
								<p><?php _e('Our Premium Support option is ideal if you have insufficient time to fix any problems you may have or simply don\'t know much about coding, we not only offer techinical support and access to our latest betas but we will also install the plugin and set it up on your site if needed. We also do customisations of the plugin for some premium members, although we recommend contacting us via our <a href="http://pixopoint.com/contact/">contact form</a> for such requests as some customisations may require extra payment depending on their complexity.','pixopoint_theme'); ?></p>
								<p><?php _e('If you have any questions about our premium services or are looking for other help with WordPress or (X)HTML/CSS projects including theme development please do not hesitate to contact us via our <a href="http://pixopoint.com/contact/">contact form</a>. More information about our premium services is available on our site ... <strong><a href="http://pixopoint.com/premium-support/">http://pixopoint.com/premium-support/</a></strong>','pixopoint_theme'); ?></p>
								<h4><?php _e('FAQ','pixopoint_theme'); ?></h4>
								<p><?php _e('<strong>Q:</strong><em> Your plugin doesn\'t work in IE, why don\'t you fix it?</em> <br /><strong>A:</strong> The plugin does work with IE, you just haven\'t integrated it correctly. See \'Free support\' below for some tips on how to get it working with IE.','pixopoint_theme'); ?></p>
								<p><?php _e('<strong>Q:</strong><em> How do I change the menu contents?</em> <br /><strong>A:</strong> See the big tab at the top of the screen right now which says "Menu Contents"? Click that ...','pixopoint_theme'); ?></p>
								<p><?php _e('<strong>Q:</strong><em> How do I change the colour/font/whatever in my menu?</em> <br /><strong>A:</strong> Click the \'home\' tab above and click the \'Enable Editing Panel\' button, then visit one of the main pages of your site which should now show an editing panel at the bottom of the screen.','pixopoint_theme'); ?></p>
								<p><?php _e('<strong>Q:</strong><em> How do I get a fully customised version?</em> <br /><strong>A:</strong> Leave a message on the <a href="http://pixopoint.com/contact/">PixoPoint Contact Page</a> with your requirements and we will get back to you ASAP with pricing information. Alternatively you can sign up for our <a href="http://pixopoint.com/premium-support/">Premium Support</a> option which gives you access to our dropdown, flyout and slider menu CSS generator, plus access to our premium support forum.','pixopoint_theme'); ?></p>
								<p><?php _e('<strong>Q:</strong><em> Why can\'t the plugin do X, Y or Z?</em> <br /><strong>A:</strong> It probably can, we just haven\'t supplied instructions on how to do it. If you have any requests, then please leave them in the <a href="http://pixopoint.com/forum/index.php?board=4.0">PixoPoint menu support board</a>. We often update the plugin with new functionality and we\'re far more likely to include the functionality you want if we know there is a demand for it already.','pixopoint_theme'); ?></p>
								<h4><?php _e('Free support','pixopoint_theme'); ?></h4>
								<p><?php _e('If you follow all of the instructions here, activate the plugin and find the menu is appearing on your site but looks all messed up, then the problem is probably caused by a clash between your themes CSS and plugins CSS. These problems can usually be remedied by removing the wrapper tags which surround the menu in your theme. For example, most themes will have some HTML such as <code>&lt;div id="nav"&gt;&lt;ul&gt;&lt;?php wp_list_pages(); ?&gt;&lt;/ul&gt;&lt;/div&gt;</code> which contains the existing themes menu. By placing the <code>wp_page_menu()</code> code between those DIV tags, the menu will often interact with that DIV tag. The solution is to either remove the DIV tag or to alter it\'s CSS so that it doesn\'t interact with the menu.','pixopoint_theme'); ?></p>
								<p><?php _e('If you require further help with the plugin, please visit the <a href="http://pixopoint.com/pixopoint-menu/">PixoPoint Menu Plugin page</a> or the <a href="http://pixopoint.com/forum/index.php?board=4.0">PixoPoint menu support board</a>.','pixopoint_theme'); ?></p>
								<p><?php _e('We are happy to answer questions but we\'ve been noticing lately that more time is being spent teaching people what to ask rather than actually answering anything. So before posting questions on the PixoPoint support forum, please read the following tips to help us answer your questions faster.','pixopoint_theme'); ?></p>
								<ul>
									<li><?php _e('Where is your CSS?','pixopoint_theme'); ?></li>
									<li><?php _e('What modifications have you made to the CSS?','pixopoint_theme'); ?></li>
									<li><?php _e('What browsers are you having problems with?','pixopoint_theme'); ?></li>
									<li><?php _e('What is the URL for your site?','pixopoint_theme'); ?></li>
									<li><?php _e('Provide a link to the problem. Most problems can not be answered without actually seeing your site. If you don\'t want to install the plugin on your live site and don\'t have a test site to show us, then view the source code in your browser when you do have the plugin installed, save it to an HTML file and upload that somewhere so that we can see what the page looks like.','pixopoint_theme'); ?></li>
									<li><?php _e('Do not bother providing us with HTML and/or CSS code snippets (without a link). There is very little we can do without seeing the entire page as most problems are caused by an obscure piece of CSS somewhere else on the page.','pixopoint_theme'); ?></li>
									<li><?php _e('Let us know if you have modified the CSS. If it is modified we are unlikely to offer support for free. Rummaging through other peoples code is too time consuming sorry.','pixopoint_theme'); ?></li>
									<li><?php _e('If you are not storing your CSS in the WP plugins settings page, let us know which <strong>exact</strong> file it is in. Searching through a dozen CSS files in your theme trying to find your menu code is not fun.','pixopoint_theme'); ?></li>
								</ul>
								<h4><?php _e('Other plugins we recommend','pixopoint_theme'); ?></h4>
								<p><?php _e('The <a href="http://wordpress.org/extend/plugins/my-category-order/">My Category Order Plugin</a> by <a href="http://www.geekyweekly.com/">froman118</a> allows you to set the order of \'WordPress Categories\' through a drag and drop interface. This replaces the need to use the clumsy default method of setting category orders built into WordPress.','pixopoint_theme'); ?></p>
								<p><?php _e('If you know of any other plugins which work well with the \'PixoPoint Menu plugin\' then please us know about them.','pixopoint_theme'); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<?php /* Credits tab */ ?>
			<div class="tabbertab">
				<h2><?php _e('Credits','pixopoint_theme'); ?></h2>
				<div class="clear"></div>
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
						<tr>
							<th style="width:120px" scope="col"><?php _e('Credits','pixopoint_theme'); ?></th>
							<th scope="col"><?php _e('Description','pixopoint_theme'); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col" colspan="2"><?php _e('Thank you to everyone who has helped with the development of the PixoPoint Menu Plugin.','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class='inactive'>
							<td class='name'><a href="http://transientmonkey.com/"><?php _e('Greg Yingling','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Greg Yingling is the author of the WP SlimBox2 plugin and contributed a significant proportion of the code in the PixoPoint Menu plugin. Greg also contributed heavily to the <a href="http://pixopoint.com/multi-level-navigation-plugin/">Multi-level Navigation plugin</a> which the PixoPoint Menu plugin was based on.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://blog.mortia.org.uk/index.php/2009/07/16/pixopoint-menu-widget/"><?php _e('Andy Mortia','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Andy Mortia created the <a href="http://blog.mortia.org.uk/index.php/2009/07/16/pixopoint-menu-widget/">PixoPoint Menu Widget</a> which can add the menu for the PixoPoint Menu plugin via a widget.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://www.geekyweekly.com/"><?php _e('froman118','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('The <a href="http://geekyweekly.com/mypageorder">My Page Order Plugin</a> created by <a href="http://www.geekyweekly.com/">froman118</a> was used to provide the code necessary for the drag and drop page ordering interface. <a href="http://www.geekyweekly.com/">froman118</a> is also the developer of the <a href="http://geekyweekly.com/mycategoryorder">My Category Order plugin</a> which is ideally suited for use with the PixoPoint Menu plugin. If you appreciate these features we recommend sending a donation to <a href="http://geekyweekly.com/mycategoryorder">froman118\'s beer fund</a>.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<?php /*
						<tr class='inactive'>
							<td class='name'><a href="http://www.moobi.com.tr/"><?php _e('Eren Yasarkurt','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Eren Yasarkurt is a software developer based in Turkey. Eren is better known as <a href="http://www.simplemachines.org/community/index.php?action=profile;u=6563">Forsakenlad</a> and has been a prolific developer of SMF themes and helped considerably with the development of the server side systems which integrate into the PixoPoint Menu plugin.','pixopoint_theme'); ?></p>
							</td>
						</tr>*/ ?>
						<tr class='inactive'>
							<td class='name'><a href="http://wptavern.com/"><?php _e('Jeff Chandler','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Jeff Chandler (aka WordPress Community Superstar) is the man behind <a href="http://wptavern.com/">WPTavern.com</a>, a small but busy site frequented by some of the most talented WordPress developers around. If you would like to learn more about WordPress and how it can be modified to suit your needs, you should head over to the tavern to see what the latest news is in the WordPress world. The tavern also features an active forum for discussing various WordPress related issues.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://pmob.co.uk/"><?php _e('Paul O\'Brien','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Paul O\'Brien is an absolute master of HTML/CSS coding. He is the co-author of the <a href="">Ultimate CSS Reference</a> and can be found posting in the CSS board at <a href="http://sitepoint.com/">SitePoint.com</a> as well as at his personal site <a href="http://pmob.co.uk/">pmob.co.uk</a>. Paul was instrumental in ironing out the bugs in the CSS coding engine which provides the PixoPoint Menu plugin with it\s easy to use design editing capabilities.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://themehybrid.com/"><?php _e('Justin Tadlock','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Justin Tadlock\'s Theme Hybrid framework is one of the most popular WordPress theme frameworks and is ideally suited to use with the PixoPoint Menu plugin. Simply install the Hybrid theme and the PixoPoint Menu plugin will automatically be displayed on your site (no theme editing necessary),','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://wp-fun.co.uk/"><?php _e('Andrew Rickman','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Andrew Rickman is an expert in programming for WordPress. His blog <a href="http://wp-fun.co.uk/">http://wp-fun.co.uk/</a> is a great resource for WordPress tips. Andrew provided assistance in developing the drag and drop functionality in the PixoPoint Menu plugin.','pixopoint_theme'); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="widefat" cellspacing="0" id="active-plugins-table">
					<thead>
						<tr>
							<th style="width:120px" scope="col"><?php _e('Recommended plugins','pixopoint_theme'); ?></th>
							<th scope="col"><?php _e('Description','pixopoint_theme'); ?></th>
						</tr>
					</thead>
					<tfoot>
						<tr>
							<th scope="col" colspan="2"><?php _e('Here are selection of plugins we recommend using on your site. Note: These plugins were developed by friends of PixoPoint, they are not paid advertisements.','pixopoint_theme'); ?></th>
						</tr>
					</tfoot>
					<tbody class="plugins">
						<tr class='inactive'>
							<td class='name'><a href="http://pixopoint.com/wp-slimbox2-plugin/"><?php _e('WP Slimbox2 plugin','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Creates animated Javascript powered overlay popups of your images. Includes a plethora of options to allow you to modify it\'s behaviour. The use of jQuery allows for better integration with other plugins and a smaller size and therefore faster page loads.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://www.instinct.co.nz/wordpress-wiki-plugin/"><?php _e('WordPress Wiki','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('Tired of using Media Wiki? Sick of fighting with WordPress to get your own documentation project happening? If so then WordPress Wiki Plugin is the answer for you. Adds Wiki functionality to your WordPress powered website.','pixopoint_theme'); ?></p>
							</td>
						</tr>
						<tr class='inactive'>
							<td class='name'><a href="http://www.instinct.co.nz/e-commerce/"><?php _e('WP e-Commerce','pixopoint_theme'); ?></a></td>
							<td class='desc'>
								<p><?php _e('An elegant easy to use fully featured shopping cart application suitable for selling your products, services, and or fees online. Perfect for: Bands & Record Labels, Clothing Companies, Crafters & Artists, Books, DVDs & MP3 files.','pixopoint_theme'); ?></p>
							</td>
						</tr>
					</tbody>
				</table>
